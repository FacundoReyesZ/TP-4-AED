...
def validar_car_cant2(n, lim_izq, lim_der, msn):
    flag_cant = True
    flag_car = True
    flag = True
    car = ""
    ...
    while flag:
        for i in x:
            if i.isdigit():
                ...
                x = str(input("ERROR... Porfavor " + msn))
                break
        if car == x[-1]:
            flag = False

    while flag_cant and flag_car:
        if len(x) == n:
            flag_cant = False
            if x.isdigit():
                if lim_izq <= int(x) <= lim_der:
                    flag_car = False
            else:
                x = str(input("ERROR... Porfavor " + msn))
        else:
...
def cargar_registro(arr):
    ...
    identificador = validar_car_cant2(10, 0000000000, 9999999999, "Ingrese codigo (numero entero de 10 caracteres):")
    ...
    distancia = validar_car_cant2(3, 000, 999, "Ingrese distancia (numero entero de 3 caracteres):")
...
def op_4(arr):
    if validar_vector(arr):
        print("No hay tickets registrados")
    else:
        patente = validar_n_caracteres(7, "ingrese patente(combinacion letras y numeros de 7 caracteres):")
        pais_cabina = validar_entre(0, 4, "ingrese pais cabina(0: Argentina - 1: Bolivia - 2: Brasil - "
                                          "3: Paraguay - 4: Uruguay):")
        pos = busq_secuencial(arr, patente)
        if pos != -1:
            if pais_cabina == arr[pos].pais_cabina:
                print("\nLa patente existe\n")
                print(arr[pos])
            else:
                print("La patente existe pero no paso por esa cabina")
        else:
...
def mayor_monto(arr):
    if validar_vector(arr):
        print("Debe realizar antes el punto 7")
    else:
        TIPOS_VEHICULO = ['Moto', 'Automovil', 'Camión']
        contadores = [0] * len(TIPOS_VEHICULO)
        imp_total = 0
